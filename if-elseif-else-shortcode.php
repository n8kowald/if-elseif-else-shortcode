 ...
 * Description:     Use if-elseif-else conditions in your editor content with a shortcode.
...
// Register shortcode
add_shortcode( 'if', 'if_elseif_else_statement' );
 ...
 * @return string
 */
function if_elseif_else_statement( $atts, $content ) {
    if ( empty( $atts ) ) {
        return '';
    }
    $pattern_else      = '[else]';
    ...
    if ( ! is_valid_callable( $callable ) ) {
        return 'If shortcode error: [if] argument must be callable';
    }
    ...
    if ( ! $if &&
        ...
        foreach ( $matches as $match ) {
            ...
            if ( ! is_valid_callable( $callable ) ) {
                return 'If shortcode error: [elseif] argument must be callable';
            }
 ...
 * @return bool
 */
function is_valid_callable( $callable ) {
	return is_callable( $callable )
		&& in_array( $callable, get_allowed_callables() );
}
 ...
 * @return mixed|void
 */
function get_allowed_callables() {
    $whitelist = [
		'comments_open',
		'is_404',
		'is_admin',
